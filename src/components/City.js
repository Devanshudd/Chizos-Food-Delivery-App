...
const City = ({ handleVisible, X }) => {
  ...
  return (
    ...
    <div className="bg-slate-100 w-[28rem] sm:w-[34rem] md:w-[40rem] h-screen overflow-y-scroll overflow-x-hidden pb-12 custom-scrollbar">
      ...
      <button
        className="ml-16 m-4 block"
        onClick={() => handleVisible((prev) => !prev)}
      >
        {" "}
        <X className="w-6 h-6" />{" "}
      </button>
      <input
        type="text"
        className="p-4 mt-4 ml-4 sm:ml-16 text-lg rounded-lg bg-slate-100 border border-black w-2/3"
      ...
      <div className="ml-4 sm:ml-24 mt-8">
        {searchLocation?.map((location) => (
          ...
          >
            ...
            >
              <h4 className="text-lg hover:text-red-600">{location?.name}</h4>
