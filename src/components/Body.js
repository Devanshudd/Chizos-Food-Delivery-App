  ...
  const fetchData = async () => {
    try {
      ...
      const data = await fetch(
        `${SERVER_URL}/api/swiggy/getData?lat=${latitude}&lng=${longitude}`
      );
    ...
    <div className="body mt-0 overflow-x-hidden">
      ...
      <div className="filter m-4 p-4 md:flex justify-center">
        <input
          ...
          type="text"
          className="p-2 ml-[10%] sm:ml-[20%] md:ml-4 m-4 w-[16rem] sm:w-[24rem] border-2 border-solid border-gray-500 rounded-xl"
          value={searchText}
          onChange={(e) => {
        ...
        <div className="flex justify-center">
          <button
            className="px-4 py-2 my-4 bg-green-100 rounded-lg"
            onClick={() => {
              const filteredRes = resLists?.filter(
                (res) =>
                  res?.info?.name
                    .toLowerCase()
          ...
          <button
            className="px-4 py-2 my-4 mx-8 bg-gray-100 rounded-lg"
            onClick={() => {
              setFilteredList(
                resLists.filter((resData) => resData?.info?.avgRating > 4)
              );
            }}
          >
      ...
      <h3 className="sm:font-bold text-2xl ml-8 sm:ml-16 md:ml-24">
