    ...
    <div>
      {items.map((item) => (
        <div
          ...
          className="p-2 m-2 border-b-2 border-gray-300 text-left flex justify-between relative pb-8"
        >
          ...
          <div className="w-1/2 xs:w-1/2 sm:w-1/3 lg:w-1/4 xl:w-1/5 p-6 relative">
            {addBtn && (
              <button
                className="absolute right-4 px-2 bottom-4 font-bold bg-slate-50 text-red-700 border-gray-200 border-2 rounded-lg shadow-lg"
          ...
          {!addBtn && (
            <div className="absolute px-2 right-1 bottom-2 bg-slate-50 text-gey-700 border-gray-200 border-1 rounded-lg">
              Quantity={item?.quantity}
            </div>
          )}
          {!addBtn && (
            <div className="absolute px-2 left-0 bottom-2 bg-slate-50 text-gey-700 border-gray-200 border-1 rounded-lg">
              Total Price=
              {item?.quantity *
                (item?.card?.info?.price
                  ? item?.card?.info?.price / 100
